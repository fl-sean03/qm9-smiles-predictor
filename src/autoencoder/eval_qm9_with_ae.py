# src/autoencoder/eval_qm9_with_ae.py
#!/usr/bin/env python3
"""
Chunked evaluation of QM9 FNNs trained on AE embeddings.
"""
import os
import argparse
import json
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate QM9 AE-run models vs. paper MAEs")
    p.add_argument("--embeddings",    required=True,
                   help=".npy file of AE embeddings (aligned to descriptors_csv)")
    p.add_argument("--descriptors_csv", required=True,
                   help="Cleaned descriptors CSV (with orig_index for alignment)")
    p.add_argument("--targets_csv",     required=True,
                   help="QM9 targets CSV (with orig_index)")
    p.add_argument("--run_dir",         required=True,
                   help="Dir containing model/, scalers/ subfolders")
    p.add_argument("--out_dir",         required=True,
                   help="Where to write per-prop outputs & summary CSV")
    return p.parse_args()

def load_embeddings(emb_np, desc_csv):
    # load embeddings and orig_index
    H = np.load(emb_np)
    df_desc = pd.read_csv(desc_csv)
    idx = df_desc['orig_index'].values
    if len(idx) != H.shape[0]:
        raise ValueError(f"Row count mismatch: {len(idx)} vs {H.shape[0]}")
    df_emb = pd.DataFrame(H, index=idx)
...
def main():
    args = parse_args()
    # load data
    df_emb = load_embeddings(args.embeddings, args.descriptors_csv)
