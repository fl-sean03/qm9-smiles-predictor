# src/hybrid/viz_hybrid.py
#!/usr/bin/env python3
"""
Visualization for QM9 hybrid-run evaluation results.
Generates per-prop parity/residuals and aggregated comparison.
"""
import os
import glob
import json
...
def parse_args():
    p = argparse.ArgumentParser(description="Hybrid-run QM9 visualizations")
...
def main():
    ...
    for metaj in glob.glob(os.path.join(args.eval_dir, '*', 'metrics.json')):
        ...
        if not os.path.exists(preds_csv): continue
    ...
    ax.set_ylabel('MAE'); ax.set_title('MAE: paper vs hybrid'); ax.legend(['paper','hybrid']); ax.grid(axis='y', linestyle='--', alpha=0.5)
    plt.xticks(rotation=15); plt.tight_layout(); fig.savefig(os.path.join(args.comparison_dir,'MAE_compare.png'), dpi=300); plt.close(fig)
    ...
    ax.axhline(0, linestyle='--', color='gray'); ax.set_ylabel('% Δ vs paper'); ax.set_title('% Improvement hybrid')
    ax.grid(axis='y', linestyle='--', alpha=0.5); plt.xticks(rotation=15); plt.tight_layout(); fig.savefig(os.path.join(args.comparison_dir,'delta_pct.png'), dpi=300); plt.close(fig)

    # R2 & RMSE bars
    for metric_name in ['R2','RMSE']:
        fig, ax = plt.subplots(figsize=(8,4))
        bar_with_labels(df['prop'], df[metric_name], ax, fmt="{:.3f}" if metric_name=='R2' else "{:.4f}")
        ax.set_ylabel(metric_name); ax.set_title(f"{metric_name} across props (hybrid)"); ax.grid(axis='y', linestyle='--', alpha=0.5)
        plt.xticks(rotation=15); plt.tight_layout(); fig.savefig(os.path.join(args.comparison_dir,f"{metric_name}_across_props.png"), dpi=300); plt.close(fig)

    # summary tables
    df.to_csv(os.path.join(args.comparison_dir,'summary_table.csv'), index=False)
    md = ['|Property|paper MAE|hybrid MAE|RMSE|R2|Δ%|','|---|---|---|---|---|---|']
    ...
    print(f"\nWritten comparison visuals & tables to {args.comparison_dir}")
